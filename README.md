...
| 阶段 | 加什么 | 验收 | 状态 |
|---|---|---|---|
| **0** | RV64I 数据通路 + 状态机 | `rv64ui-p-*` 全过 | ✅ 50/50 |
| **1** | 逐指令对照基础设施 | 与 riscvm 的 trace 一致 | ✅ |
| **2** | CSR + M 模式陷入 | `rv64mi-p-*` 全过 | ✅ 12/16 |
...
`rv64mi` 的四个未过项是尚未实现的功能,不是缺陷:`mcsr`(只读 ID 寄存器)、
`zicntr`(计数器)、`breakpoint`(调试触发模块)、`access`(PMP)。它们在
`IsaSpec.scala` 里以**反向断言**记录——功能做完时测试会转为通过,反向断言随之
报错,提醒把它移进通过列表。清单不会腐烂成注释。
...
| 层 | 测什么 | 覆盖不了什么 |
|---|---|---|
| co-sim vs riscvm | 逐指令一致性 | 两边同样理解错误 |
| known-answer | 对着规范的具体值 | 只覆盖想到的点 |
| riscv-tests | 外部、规范作者维护 | 只到规范要求的粒度 |
| 慢速存储器等价 | 延迟不改变计算结果 | — |

第一层的参考模型是我们自己写的,所以第三层不可替代:**co-sim 测量一致性,
riscv-tests 测量符合性**。

第四层是第 0 阶段"存储器接口带完整握手"那个决定的持续回归:每个 co-sim 程序都跑
两遍(0 周期和 7 周期延迟),结果必须逐条相同。
